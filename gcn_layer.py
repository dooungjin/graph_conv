...
class GraphConv(MessagePassing):
    ...
    """
    def __init__(self, node_channels, edge_channels, out_channels, drop_prob):
    ...
    def update(self, aggr_out, x):
        ...
        return F.relu(aggr_out + torch.matmul(x, self.node_weight) + self.bias)
...
class GraphNetLayers(torch.nn.Module):
    ...
    """
    def __init__(self, input_dim, output_dim, num_layers, drop_prob):
        """
        Constructs a new `GraphNet` with specified configuration.
        """
        super(GraphNetLayers, self).__init__()
        gcn_layers = [GraphConv(input_dim, 2, output_dim, drop_prob)]

        if num_layers > 1:
            for i in range(num_layers - 1):
                gcn_layers.append(GraphConv(output_dim, 2, output_dim, drop_prob))
...
def generate_random_data(length, input_dim):
        x = torch.randn(length, input_dim)
        edge_index = torch.stack((torch.repeat_interleave(torch.arange(length), 10), torch.randint(0, length, (length * 10,))))
        edge_attr = torch.randn(length * 10, 2)
